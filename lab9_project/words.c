...
void words_tick() {
  ...
  case gameModeSt:
    ...
    if ((buttonVal & BUTTONS_BTN3_MASK) == BUTTONS_BTN3_MASK) {
      ...
      setWord();
      gameMode = 3;
      displayWords(gameMode, firstWordBlank, secondWordBlank, thirdWordBlank,
                   false);
  ...
  case correctSt:
    ...
    if (gameMode == 3) {
      if (rightChoice ==
          (strlen(firstWord) + strlen(secondWord) + strlen(thirdWord))) {
        endScreen(guesses, TIMER, false);
        currentState = endSt;
      } else
        currentState = chooseSt;
...
void setWord() {
  ...
  char wordfirst[][11] = {"ARMCHAIRS", "AUNTS",    "BALLS", "BERMUDAS",
                          "BEANS",     "BALLOONS", "BEARS", "BLOUSES",
                          "BEDS",      "BABIES",   "PSST"};
  char wordsecond[][10] = {"AFFORD", "AGREE",   "AIM", "APPEAR", "ATTEMPT",
                           "ASK",    "ARRANGE", "BEG", "BEGIN",  "CARE"};
  char wordthird[][11] = {"ARMCHAIRS", "AUNTS",    "BALLS", "BERMUDAS",
                          "BEANS",     "BALLOONS", "BEARS", "BLOUSES",
                          "BEDS",      "BABIES",   "PSST"};
...
void locationToLetter() { letterChoice[guesses] = 64 + point.x + point.y; }
  ...
  if (point.y == 0) {
    if (correct)
      display_drawChar(CONFIG_BOX_KEYBOARD_X_0 + 5 +
                           CONFIG_LINE_VERTICLE_LETTER_SPACE * (point.x - 1),
                       CONFIG_LETTER_Y_TOP, 64 + point.x + point.y,
                       CONFIG_COLOR_USED_TEXT, 1, 3);
    else
      display_drawChar(CONFIG_BOX_KEYBOARD_X_0 + 5 +
                           CONFIG_LINE_VERTICLE_LETTER_SPACE * (point.x - 1),
                       CONFIG_LETTER_Y_TOP, 64 + point.x + point.y,
                       CONFIG_COLOR_MAN, 1, 3);
  } else if (correct)
    display_drawChar(CONFIG_BOX_KEYBOARD_X_0 + 5 +
                         CONFIG_LINE_VERTICLE_LETTER_SPACE * (point.x - 1),
                     CONFIG_LETTER_Y_BOTTOM, 64 + point.x + point.y,
                     CONFIG_COLOR_USED_TEXT, 1, 3);
  else
    display_drawChar(CONFIG_BOX_KEYBOARD_X_0 + 5 +
                         CONFIG_LINE_VERTICLE_LETTER_SPACE * (point.x - 1),
